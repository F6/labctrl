...
def calculate_dx(value: float, unit: str, multiples: float, direction: str) -> float:
    """
    accepts a value in working unit, returns dx in mm
    """
    # coefficient: both are dx, so c = 1 mm/mm
    c = 1
    # convert dx to mm unit
    if unit == "mm":
        value = value
    elif unit == "um":
        value = value / 1000
