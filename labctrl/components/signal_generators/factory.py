...
from .bundle_bokeh import BundleBokehSignalGenerator


class FactorySignalGenerator:
    """
    This class is responsible for generating BundleSignalGenerator objects 
    ...
    def generate_bundle(self, bundle_config: dict):
        ...
        if name in self.generated:
            print(
                "[SANITY] FactorySignalGenerator: BundleSignalGenerator with name {} \
                    already generated before!".format(name))
        # if bundle_config[widget_package]==bokeh ->
        foo = BundleBokehSignalGenerator(bundle_config, self.lcfg, self.lstat)
